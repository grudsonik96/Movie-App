...
function getLocation(id) {
  if (navigator.geolocation) {
    ...
    let simulateWeatherContainer = document.getElementById(
      "simulateWeatherContainer"
    );
...
function getData(url) {
  ...
  request.onload = function () {
    ...
    weatherIcon.innerHTML = "<img src=" + weather.weather[0].icon + ".png>"; // Weather icon
  ...
  request.onload = function () {
    ...
    for (let i = 0; i < 4; i++) {
      if (movie.results[counter + i] == null) {
        posterpaths[i + counter] =
          "https://dummyimage.com/185x280/fff/000&text=Out+of+movies!";
      } else {
        if (movie.results[counter + i].poster_path == null) {
          posterpaths[i + counter] =
            "https://dummyimage.com/185x280/ffffff/00000&text=Unable+to+fetch+poster";
        } else {
          posterpaths[counter + i] =
            "https://image.tmdb.org/t/p/w185" +
            movie.results[counter + i].poster_path;
        }
      }
    }
    ...
    poster.innerHTML =
      ...
      ' class="img-thumbnail" >';
    poster2.innerHTML =
      '<img class="img-thumbnail" src=' + posterpaths[counter + 1] + ">";
    poster3.innerHTML =
      '<img class="img-thumbnail" src=' + posterpaths[counter + 2] + ">";
    poster4.innerHTML =
      '<img class="img-thumbnail" src=' + posterpaths[counter + 3] + ">";

    // print title and description
    description.innerHTML = "<i>" + movie.results[counter].overview + "<i/>";
    title.innerHTML =
      '  <h1 id="movie_title">' +
      movie.results[counter].title +
      "       " +
      '            <small class="font-italic">(' +
      movie.results[counter].release_date.slice(0, 4) +
      ")</small>\n" +
      "            </h1>\n";

    castURL =
      "https://api.themoviedb.org/3/movie/" +
      movie.results[counter].id +
      "/credits?api_key=" +
      MAPI_KEY;
    ratingURL =
      "https://api.themoviedb.org/3/movie/" +
